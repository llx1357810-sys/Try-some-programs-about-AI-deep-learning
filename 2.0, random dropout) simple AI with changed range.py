...
# notes in test : {a11: -4.67903817, a12: 5.28001362, b11: 2.52293874, b12: 2.65846244, a21: -2.71618078, a22: 3.35205077, b21: 1.87168132, b22: 2.6344191, a31: -3.9327126, a32: 4.60443643, b31: 2.1855423, b32: 2.47697591, a41: -3.19562559, a42: 3.83354076, b41: 2.01718226, b42: 2.43976794, a51: -4.52394279, a52: 5.3058142, b51: 2.3464497, b52: 2.51559711, w1: 0.10916013, w2: 0.1831286, w3: 0.03353309, w4: 0.00159874, w5: 0.088359}
...
for i in range(0,time,1):
    ...
    dropout = random.randint(0,10)

#main position and other random set
    ...
    weight1 = dictionary[w1]
    weight2 = dictionary[w2]
    weight3 = dictionary[w3]
    weight4 = dictionary[w4]
    weight5 = dictionary[w5]
 ...
 #first width
    if w_d_1_value != 1:
        ...
        if weight_new ** 2 > (weight2 + weight3 + weight4 + weight5) ** 2:
            weight_new = round(weight_now - 0.1 * n * gradient_weight_now, 8)
            n = 0.0001
        if weight_new > 0:
...
#second width
    if w_d_2_value != 2:
        ...
        if weight_new ** 2 > (weight1 + weight3 + weight4 + weight5) ** 2:
            weight_new = round(weight_now - 0.1 * n * gradient_weight_now, 8)
            n = 0.0001
        if weight_new > 0:
...
#third width
    if w_d_3_value != 3:
        ...
        if weight_new ** 2 > (weight1 + weight2 + weight4 + weight5) ** 2:
            weight_new = round(weight_now - 0.1 * n * gradient_weight_now, 8)
            n = 0.0001
        if weight_new > 0:
...
#fourth width
    if w_d_4_value != 4:
        ...
        if weight_new ** 2 > (weight2 + weight3 + weight1 + weight5) ** 2:
            weight_new = round(weight_now - 0.1 * n * gradient_weight_now, 8)
            n = 0.0001
        if weight_new > 0:
...
#fifth width
    if w_d_5_value != 5:
        ...
        if weight_new ** 2 > (weight2 + weight3 + weight4 + weight1) ** 2:
            weight_new = round(weight_now - 0.1 * n * gradient_weight_now, 8)
            n = 0.0001
        if weight_new > 0:
    ...
    if error_now < 5:
        ...
        if error_now < 2:
            very_good += 1
            if very_good > 30:
                print('The training have finished before your setting time.')
                break
            else:
                very_good = 0
    else:
